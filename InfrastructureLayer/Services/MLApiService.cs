        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            foreach (var file in request.Files ?? Enumerable.Empty<MLApiFileDto>())
            {
                ...
                {
                    _logger.LogWarning("Skipping file {FileName}; source URL returned {StatusCode}.", file.FileName, downloadResponse.StatusCode);
                    continue;
                }

                var fileBytes = await downloadResponse.Content.ReadAsByteArrayAsync(cancellationToken);
                var fileContent = new ByteArrayContent(fileBytes);
                var contentType = downloadResponse.Content.Headers.ContentType?.ToString();
